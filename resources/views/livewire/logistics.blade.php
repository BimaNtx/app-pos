<div class="p-6 space-y-6">
    ...
    <div class="flex flex-col md:flex-row gap-4 justify-between">
        <div class="relative flex-1 max-w-2xl group">
            <div
                class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors group-focus-within:text-teal-500 text-gray-400">
        ...
        <button wire:click="create"
            class="px-6 py-3.5 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-2xl shadow-lg shadow-teal-500/20 hover:shadow-teal-500/30 transition-all transform hover:-translate-y-0.5 flex items-center gap-2">
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-gray-50/50 border-b border-gray-100">
                        <th class="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Item
                        </th>
                        <th class="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider">Stok</th>
                        <th class="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider">Satuan</th>
                        <th class="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider">Stok Min.
                        </th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($logistics as $item)
                        <tr class="hover:bg-gray-50/80 transition-colors group">
                            ...
                            <td class="py-4 px-6">
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-md text-sm font-bold bg-blue-50 text-blue-600">
                                    {{ $item->stock }}
                                </span>
                            </td>
                            ...
                            <td class="py-4 px-6">
                                @if($item->stock <= $item->minimum_stock)
                                    <span
                                        class="inline-flex items-center gap-1.5 px-3 py-1 bg-red-50 text-red-600 text-xs font-bold rounded-full border border-red-100">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center gap-1.5 px-3 py-1 bg-green-50 text-green-600 text-xs font-bold rounded-full border border-green-100">
                                        <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M5 13l4 4L19 7" />
                                        </svg>
                        ...
                        <tr>
                            <td colspan="6" class="py-16 text-center text-gray-400">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900">Tidak ada item ditemukan</h3>
                                    <p class="text-sm text-gray-500 mt-1">Mulai dengan menambahkan item inventaris pertama
                                        Anda.</p>
                                </div>
                            </td>
                        </tr>
    ...
    @if($isOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" wire:click.self="closeModal">
            <div class="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl transform transition-all">
                <h3 class="text-xl font-bold text-gray-800 mb-4">{{ $isEditMode ? 'Edit Item' : 'Tambah Item Baru' }}</h3>
                ...
                <form wire:submit.prevent="store">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input wire:model="name" type="text" placeholder="Contoh: Beras, Gula, Minyak"
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div x-data="{ val: @entangle('stock') }">
                                ...
                                <div class="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                                    <button type="button" @click="val = Math.max(0, parseInt(val) - 1)"
                                        class="px-3 py-2 bg-gray-50 hover:bg-gray-100 border-r border-gray-200 text-gray-600 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M20 12H4" />
                                        </svg>
                                    ...
                                    <input wire:model="stock" type="number" min="0"
                                        class="w-full px-2 py-2 text-center border-0 focus:ring-0 appearance-none bg-white">
                                    <button type="button" @click="val = parseInt(val) + 1"
                                        class="px-3 py-2 bg-gray-50 hover:bg-gray-100 border-l border-gray-200 text-gray-600 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M12 4v16m8-8H4" />
                                        </svg>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Stok Minimum (Peringatan)</label>
                            <input wire:model="minimum_stock" type="number" min="0"
                                class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                            <p class="text-xs text-gray-500 mt-1">Stok di bawah level ini akan menampilkan peringatan.</p>
